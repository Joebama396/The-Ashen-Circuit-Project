...
python3 -m PyInstaller --noconfirm --clean --onefile --windowed --name ashen-circuit \
  ...
  --add-data "assets/characters/party_battle_v6.png:assets/characters" \
...
portable_temp="dist/The_Ashen_Circuit-${VERSION}-Linux-Portable.writing.tar.gz"
tar -czf "$portable_temp" -C build/portable The-Ashen-Circuit
gzip -t "$portable_temp"
mv "$portable_temp" "dist/The_Ashen_Circuit-${VERSION}-Linux-Portable.tar.gz"
...
zip_temp="dist/The_Ashen_Circuit-${VERSION}-Ubuntu.writing.zip"
zip -j -q "$zip_temp" \
  "dist/The_Ashen_Circuit-${VERSION}-x86_64.AppImage" \
  dist/Play-The-Ashen-Circuit.sh \
  dist/START_HERE.txt
unzip -tq "$zip_temp"
mv "$zip_temp" "dist/The_Ashen_Circuit-${VERSION}-Ubuntu.zip"
(
  cd dist
  sha256sum "The_Ashen_Circuit-${VERSION}-x86_64.AppImage" \
    Play-The-Ashen-Circuit.sh \
    "The_Ashen_Circuit-${VERSION}-Ubuntu.zip" \
    "The_Ashen_Circuit-${VERSION}-Linux-Portable.tar.gz" \
    START_HERE.txt \
